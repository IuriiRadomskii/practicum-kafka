...
function create_certs() {
    ...
    openssl req -new -key kafka-server.key -out kafka-0.csr -subj "/CN=kafka-0"
    ...
    openssl x509 -req -in kafka-0.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out kafka-0.crt -days 365
    ...
    openssl pkcs12 -export -in kafka-0.crt -inkey kafka-server.key -out kafka-0.p12 -name kafka-0 -password pass:password
    ...
    keytool -importkeystore -destkeystore ./kafka-0/kafka.keystore.jks -srckeystore kafka-0.p12 -srcstoretype PKCS12 -alias kafka-0 -storepass password -srcstorepass password
    ...
    keytool -keystore ./kafka-0/kafka.truststore.jks -alias CARoot -import -file ca.crt -storepass password -noprompt
